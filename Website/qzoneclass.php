...
{
	throw new Exception('Qzone API needs the cURL PHP extension.');
}
...
{
	throw new Exception('Qzone API needs the JSON PHP extension.');
}

/**
 * 错误码定义
 */
define('QZO_ERROR_REQUIRED_PARAMETER_EMPTY', 2001); // 参数为空
define('QZO_ERROR_REQUIRED_PARAMETER_INVALID', 2002); // 参数格式错误
define('QZO_ERROR_RESPONSE_DATA_INVALID', 2003); // 返回包格式错误
define('QZO_ERROR_CURL', 3000); // 网络错误, 偏移量3000, 详见 http://curl.haxx.se/libcurl/c/libcurl-errors.html

/**
 * 提供访问QQ空间 OpenAPI 的接口
 */
class Qzone
{
	/**
	 ...
	 * 具体API地址请参考文档 http://wiki.opensns.qq.com/wiki/qzone/user/info
	 * 注意区分正式、测试环境
	 */
	private $serverName = '';
	 ...
	 * 设置API服务器的域名
	 *
	 * @param string $serverName  Qzone OpenAPI 服务器的域名
	 */
	 ...
	 * 获取API服务器的域名
	 *
	 * @return string  Qzone OpenAPI 服务器的域名
	 */
	...
	public function api($method, $params)
	{
		...
		{
			return array(
				'ret' => QZO_ERROR_REQUIRED_PARAMETER_EMPTY,
				'msg' => 'openid is empty');
		}
		// 检查 openkey 是否为空
		...
		{
			return array(
				'ret' => QZO_ERROR_REQUIRED_PARAMETER_INVALID,
	...
	private function makeRequest($url, $params)
	{
		...
		$opts[CURLOPT_USERAGENT] = 'qzone-php-1.0';
		...
		if (false === $result)
		{
			...
			$err = array(
				'ret' => QZO_ERROR_CURL + curl_errno($ch),
				'msg' => curl_error($ch));
			curl_close($ch);
			return $err;
		}
		...
		if (is_null($result_array)) {
			return array(
				'ret' => QZO_ERROR_RESPONSE_DATA_INVALID,
				'msg' => $result);
		}
		return $result_array;
	}
	...
	{
		$result = $this->api('/qzone/user/info',
						array(
							'openid' => $openid,
							'openkey' => $openkey
						)
	 ...
	 */
	public function getFriendList($openid, $openkey)
	{
		$result = $this->api('/qzone/relation/friends',
						array(
							'openid' => $openid,
							'openkey' => $openkey
						)
	...
	public function isFriend($openid, $openkey, $input_array)
	{
		...
		$result = $this->api('/qzone/relation/is_friend',
						array(
							'openid' => $openid,
							'openkey' => $openkey,
	...
	public function getMultiInfo($openid, $openkey, $input_array = array())
	{
		...
		$result = $this->api('/qzone/user/multi_info',
						array(
							'openid' => $openid,
							'openkey' => $openkey,
	...
	{
		$result = $this->api('/qzone/user/is_setuped',
						array(
							'openid' => $openid,
							'openkey' => $openkey
						)
	...
	{
		$result = $this->api('/qzone/pay/is_vip',
						array(
							'openid' => $openid,
							'openkey' => $openkey
						)
	...
	public function getEmotion($openid, $openkey, $input_array)
	{
		...
		$result = $this->api('/qzone/user/emotion',
						array(
							'openid' => $openid,
							'openkey' => $openkey,
							'fopenids' => implode('_', $input_array['fopenids'])
